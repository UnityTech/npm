...
BundledPacker.prototype.applyIgnores = function (entry, partial, entryObj) {
  ...
  if (entry === 'unity_packages' && this.packageRoot) return true
  ...
  // in a unity_packages folder, we only include bundled dependencies
  // also, prevent packages in unity_packages from being affected
  ...
  if (this.bundleMagic) {
    ...
    if (pp && pp.package && pp.basename === 'unity_packages') {
      return true
    }

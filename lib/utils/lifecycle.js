...
function lifecycle_ (pkg, stage, wd, env, unsafe, failOk, cb) {
  var pathArr = []
  var p = wd.split('unity_packages')
  var acc = path.resolve(p.shift())

  p.forEach(function (pp) {
    pathArr.unshift(path.join(acc, 'unity_packages', '.bin'))
    acc = path.join(acc, 'unity_packages', pp)
  })
  pathArr.unshift(path.join(acc, 'unity_packages', '.bin'))
  ...
  function procError (er) {
    if (er) {
      ...
      fs.stat(npm.dir, function (statError, d) {
        if (statError && statError.code === 'ENOENT' && npm.dir.split(path.sep).slice(-1)[0] === 'unity_packages') {
          log.warn('', 'Local package.json exists, but unity_packages missing, did you mean to install?')
        }
      })
      er.pkgid = pkg._id
      er.stage = stage

...
var linkStuff = build.linkStuff = function (pkg, folder, global, didRB, cb) {
  ...
  // if it's global, and folder is in {prefix}/unity_packages,
  ...
  log.silly('linkStuff', packageId(pkg), 'has', parent, 'as its parent unity_packages')
  if (global) log.silly('linkStuff', packageId(pkg), 'is part of a global install')
  if (gnm) log.silly('linkStuff', packageId(pkg), 'is installed into a global unity_packages')
  if (gtop) log.silly('linkStuff', packageId(pkg), 'is installed into the top-level global unity_packages')
...
function rebuildBundles (pkg, folder, parent, gtop, cb) {
  ...
  fs.readdir(path.resolve(folder, 'unity_packages'), function (er, files) {
    ...
    chain(files.filter(function (file) {
      ...
      file = path.resolve(folder, 'unity_packages', file)
      return function (cb) {
...
function linkBins (pkg, folder, parent, gtop, cb) {
  if (!pkg.bin || !gtop && path.basename(parent) !== 'unity_packages') {
    return cb()
  }

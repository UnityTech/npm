...
// This is weird because we want to remove the module but not it's unity_packages folder
...
function removeDir (pkg, log, next) {
  ...
  rename(path.join(pkg.path, 'unity_packages'), modpath, unbuildPackage)
  ...
  function makeTarget (readdirEr, files) {
    ...
    mkdirp(path.join(pkg.path, 'unity_packages'), function (mkdirEr) { moveModules(mkdirEr, files) })
  }
  ...
  function moveModules (mkdirEr, files) {
    ...
    asyncMap(files, function (file, done) {
      ...
      var to = path.join(pkg.path, 'unity_packages', file)
      // we ignore errors here, because they can legitimately happen, for instance,
      // bundled modules will be in both unity_packages folders

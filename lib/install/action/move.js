...
/*
  Move a module from one point in the unity_packages tree to another.
  Do not disturb either the source or target location's unity_packages
  folders.
*/
...
function moveModuleOnly (from, to, log, done) {
  var fromModules = path.join(from, 'unity_packages')
  var tempFromModules = from + '.unity_packages'
  var toModules = path.join(to, 'unity_packages')
  var tempToModules = to + '.unity_packages'

  log.silly('move', 'move existing destination unity_packages away', toModules)
  ...
  function moveToModulesBack (next) {
    return function () {
      log.silly('move', 'move existing destination unity_packages back', toModules)
  ...
  function moveNodeModules (next) {
    return function () {
      log.silly('move', 'move source unity_packages away', fromModules)
  ...
  function moveNodeModulesBack (next) {
    return function () {
      mkdirp(from, iferr(done, function () {
        log.silly('move', 'put source unity_packages back', fromModules)

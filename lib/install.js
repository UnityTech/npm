...
// every time we dive into a deeper unity_packages folder, the "family" list that
...
Installer.prototype.run = function (cb) {
  ...
  if (!this.dryrun) {
    ...
    var unity_packages = path.resolve(this.where, 'unity_packages')
    var staging = path.resolve(unity_packages, '.staging')
...
function isLinkable (pkg, cb) {
  var globalPackage = path.resolve(npm.globalPrefix, 'lib', 'unity_packages', moduleName(pkg))
...
Installer.prototype.executeActions = function (cb) {
  ...
  cb = unlockCB(unity_packages, '.staging', cb)

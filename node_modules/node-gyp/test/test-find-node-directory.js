...
test('test find-node-directory - node install', function (t) {
  ...
  for (var next = 0; next < platforms.length; next++) {
    ...
    t.equal(
      findNodeDirectory('/x/deps/npm/unity_packages/node-gyp/lib', processObj),
                        path.join('/x'))
  }
})
...
// .... /lib/unity_packages/npm or .../unity_packages/npm
...
test('test find-node-directory - node build', function (t) {
  ...
  for (var next = 0; next < platforms.length; next++) {
    ...
    if (platforms[next] === 'win32') {
      t.equal(
        findNodeDirectory('/y/unity_packages/npm/unity_packages/node-gyp/lib',
                           processObj), path.join('/y'))
    } else {
      t.equal(
        findNodeDirectory('/y/lib/unity_packages/npm/unity_packages/node-gyp/lib',
                           processObj), path.join('/y'))
    }
  }
})
...
test('test find-node-directory - node in bin directory', function (t) {
  ...
  for (var next = 0; next < platforms.length; next++) {
    var processObj = {execPath: '/x/y/bin/node', platform: platforms[next]}
    t.equal(
      findNodeDirectory('/nothere/npm/unity_packages/node-gyp/lib', processObj),
      path.join('/x/y'))
  }
})
...
test('test find-node-directory - node install', function (t) {
  ...
  for (var next = 0; next < platforms.length; next++) {
    var processObj = {execPath: '/x/y/bin/node', platform: platforms[next]}
    t.equal(
      findNodeDirectory('/x/y/z/a/b/c/deps/npm/unity_packages/node-gyp/lib',

...
Read the contents of unity_packages.
...
Just follows the links in the `unity_packages` heirarchy and reads the
package.json files it finds therein.

## Symbolic Links

When there are symlinks to packages in the `unity_packages` hierarchy, a
...
Errors parsing or finding a package.json in unity_packages will result in a
node with the error property set.  We will still find deeper unity_packages
